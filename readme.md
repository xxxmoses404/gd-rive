...
GD-Rive doesn't intend to replace other dialogue managers, but to provide an alternative for specific use cases where a fluid narrative dialogue is preferred. Leveraging RiveScript essentially transforms any character (enemy, npc, companion, etc) in your game into a story and game world immersive chat bot. Instead of dialogue trees and branches, it provides a non-linear, conversation driven dialogue engine.
...
- Use `Engine.get_singleton()` internally to resolve singleton access.
